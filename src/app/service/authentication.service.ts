...
export class AuthenticationService {
  ...
  login(credentials: Credentials) {
    return this.http.post(`${environment.apiUrl}/api/User/login`, credentials)
      .pipe(
        ...
        tap((tokens: any) => (this.tokensStorage.accessToken= tokens.access_token)),
        tap((tokens: any) => (this.tokensStorage.refreshToken = tokens.refresh_token)),
        tap(() => this.saveTokens()),
  ...
  loadLoggedUser() {
    ...
    return this.http.get<User>(`${environment.apiUrl}/api/User/me`, { headers: this.settings.getHeaders() });
  }
